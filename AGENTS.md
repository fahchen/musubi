...
- The Rust client crates live under `crates/<name>/` in a Cargo workspace rooted at `Cargo.toml`: `phoenix-channel` (transport), `musubi-state` (the retained reactive state tree — no network, no UI, no runtime), `musubi-client` (runtime-free core — tokio must never reach it), `musubi-client-tokio` (the tokio seams). Edition 2024, MSRV 1.85, `publish = false`, and `version`/`edition`/dependency versions inherited from `[workspace.package]`. The crates ride the Hex tarball via `package/0` `:files` (with the root `Cargo.toml`) so a consumer path-depends on `../deps/musubi/crates/<name>`. Gates: `cargo fmt --all --check`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo test --workspace` on stable and on 1.85 — all in the `rust` CI job, none in `mix precommit`. Normative designs: `docs/rust-client.md` (transport, mount, commands, events, uploads, cache) and `docs/rust-reactive-state.md` (the data plane: `musubi-state`, `Mounted::state()`, the handle vocabulary, and the narrowed public surface)
- Wire fixtures for the Rust client live in `crates/musubi-client/tests/fixtures/*.json` and are produced by `mix musubi.capture_wire` (`test/support/mix/tasks/musubi.capture_wire.ex`, preferred env `:test`). The harness it drives — endpoint, `Musubi.Socket`, fixture root stores, recorder, scenario list — is `test/support/wire_capture/`, and `test/musubi/transport/connection_channel_test.exs` drives the **same** modules: one mechanism, not two (`docs/rust-client.md` §12). Output is canonical JSON (sorted keys, two-space indent, trailing newline) with server-issued upload entry refs renumbered, so `mix musubi.capture_wire` and `git diff --exit-code` are a drift gate; `test/musubi/wire_capture_test.exs` asserts the checked-in files match the live server. A fixture store must render nothing non-deterministic — no timestamps, pids or random ids — and upload scenarios use external-uploader mode because channel-mode tokens are signed per run. The other half is `crates/musubi-client/tests/fixtures.rs`, which replays every fixture through a real `Connection`: it enumerates the directory (a new scenario is replayed as soon as it is captured), performs the client call each `out` frame names instead of injecting the frame, feeds every `in` frame verbatim, and asserts the root's `state().value()` equals the server-authored `expected_state`
