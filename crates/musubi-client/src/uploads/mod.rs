...
//! lets [`PatchEnvelope`](crate::PatchEnvelope) decode `upload_ops` without
//! pulling the transfer machinery in — and only the control plane reaches into
//! a cell. The registry does name two control-plane types, `EntryTransport` and
...
pub use self::ops::{
    EntryStatus, UploadAccept, UploadConfig, UploadEntry, UploadError, UploadErrorCode, UploadOp,
    UploadStatus,
};
pub use self::registry::{Upload, UploadHandle, Uploads};
