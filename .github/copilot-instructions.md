...
- **Codegen drift.** When a diff edits a `state do` block (or `attr`/`command`/`event`/`upload` declarations) on a Musubi module, **both** generated bundles must be regenerated and committed: the TypeScript one at `priv/codegen/ts/musubi.d.ts` (or the consumer path under `config :musubi, :ts_codegen_output_path`) and the Rust one under `config :musubi, :rust_codegen_output_path`. The `:musubi_ts` / `:musubi_rust` Mix compilers keep them in sync on every `mix compile` once added to `compilers:`; `mix compile.musubi_ts --check` and `mix compile.musubi_rust --check` both run in `mix precommit` and return a `Mix.Task.Compiler.Diagnostic` on drift. A CI failure there means the diff missed regenerating a bundle. Adding a new field-type AST shape (e.g. a non-trivial union, a new `stream(T)` site, a new cross-module reference) should also surface a `TypeRenderer` test in `test/musubi/codegen/type_script/type_renderer_test.exs` and its Rust sibling in `test/musubi/codegen/rust/type_renderer_test.exs`. Discovery is via per-module manifest entries written by `Musubi.Plugin.Codegen`'s injected `@after_compile {Musubi.Codegen.Manifest, :__after_compile__}` callback (the callback uses `Macro.expand/2` against `env.aliases` to fully-qualify cross-module references before serializing). There is no persisted `:__musubi_ts__` attribute and no beam scan — diffs that drop the plugin from the chain will silently empty the codegen output.
