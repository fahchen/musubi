...
Both renamed modules are internal and pre-1.0:
...
So the rename lands as one standalone preparatory commit with **no deprecation
shim**:
...
orphan. Nothing reads it and `Mix.Tasks.Compile.MusubiTs.manifests/0` no longer
points at it, but `mix clean` does **not** remove it — `clean/0` `rm_rf`s
`Manifest.target_dir()`, which after the rename is the *new* directory. The
orphan survives until the next `_build` wipe. No
code change is required in any consumer app; the next `mix compile` restamps
into `musubi-codegen/` because `@after_compile` fires on recompilation of every
`state do` module (which the plugin change itself forces).

Two additional generalizations belong in the same commit, because both targets
...
`Mix.Tasks.Compile.MusubiRust`, `use Mix.Task.Compiler`,
`@compiler_name "musubi_rust"`, compiler atom `:musubi_rust`.
...
// Prelude: re-exports only. The shared runtime types are owned by the client
// crate (§4.5); this module exists so the rest of the bundle can name them
// through one depth-correct `super::`-chained path.
pub mod musubi {
...
(`src/musubi_gen.rs` + `mod musubi_gen;`) or via `include!`. Document that in
